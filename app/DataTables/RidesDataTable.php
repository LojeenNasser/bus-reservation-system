...
class RidesDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->editColumn('bus', function (Ride $ride) {
                return $ride->bus->name;
            })
            ->editColumn('departure_time', function (Ride $ride) {
                return optional($ride->departure_time)->format('H:i') ?? '-';
            })
            ->editColumn('ride_date', function (Ride $ride) {
                return optional($ride->ride_date)->format('Y-m-d') ?? '-';
            })
            ->editColumn('auto_confirm', function (Ride $ride) {
                return $ride->auto_confirm ? 'Yes' : 'No';
            })
            ->editColumn('updated_at', function (Ride $ride) {
            ...
            ->rawColumns(['route'])
            ->addColumn('ride_type', function (Ride $ride) {
                return $ride->ride_date ? 'Single' : 'Cyclic';
            })
            ->addColumn('state', function (Ride $ride) {
                return $ride->isActive() ? 'Active' : 'Inactive';
            })
            ...
            ->orderColumn('ride_type', function ($query, $order) {
                return $query->orderBy('ride_date', $order);
            });
    }
