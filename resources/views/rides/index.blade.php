            ...
            <div class="col">
                <div class="card bg-dark-lighter text-white mt-3">
                    <div class="card-header bg-dark-lightest d-flex">
                        <div class="col-sm-7">
                            <h4 class="m-0">
                                Rides from {{ $startLocation->name }} to {{ $endLocation->name }}
                            </h4>
                        </div>
                                ...
                                <table class="table text-white">
                                    ...
                                    @foreach ($rides as $ride)
                                        <tr>
                                            ...
                                            <td>{{ $ride->getArrivalTimeToLocation($startLocation->id) }}</td>
                                            <td>{{ $ride->getArrivalTimeToLocation($endLocation->id) }}</td>
                                            <td>
                                                @php
                                                    $perc = $ride->bus->seats == 0 ? 100 : $ride->booked_seats * 100 / $ride->bus->seats;
                                                @endphp
                                                <div class="progress">
                                                    <div class="progress-bar {{ $perc == 100 ? 'bg-danger' : ($perc >= 85  ? 'bg-warning' : '') }}"
                                            ...
                                            <td>
                                                <a class="btn btn-primary"
                                                   href="{{ route('bookings.create', [$ride->id, $startLocation->id, $endLocation->id, request()->get('date')]) }}">
                                                    Book This Ride
                                                </a>
                                            </td>
                                        </tr>
